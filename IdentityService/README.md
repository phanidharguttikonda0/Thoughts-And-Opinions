...
*   **Authentication & Registration**: Validated user creation logic mapping to proper database saves, securely hashing passwords, and generating JWT tokens correctly on both successful registrations and sign-ins. Ensured exceptions like `UserExistsException` (Username or Email) and `InCorrectCredentials` properly interrupt bad logins.
*   **Profile Management**: Thoroughly tested `updateProfile` for **all combinations of partial updates** (e.g. updating just a bio, just a name, or all fields at once) to ensure non-provided fields are never unintentionally overwritten with nulls. Tested user lookups and search logic.
*   **Social Graph**: Confirmed that `followUser` properly establishes bidirectional references and accurately tracks follower/following counts, while rejecting already followed states (`AlreadyFollowingException`) or non-existent unfollow attempts (`NotFollowingException`).

### 2. Edge Cases Verified
*   **Cursor Pagination Safety**: Specifically validated that fetching follower and following lists correctly leverages the `last cursor` generated by previous queries, ensuring the next paginated chunk is pulled accurately without skipping users or returning duplicates.
*   **Null Cursor Handling**: Added safeguards and test cases proving that providing a `null` cursor (or missing string on the gRPC transport) is gracefully handled by initializing a baseline `OffsetDateTime.MIN` cursor, ensuring the first page of results is delivered without NullPointerExceptions.

### 3. Validation Results
- **Total Test Cases**: 20
- **Success Rate**: 100% (20/20 passed, 0 failures, 0 errors)
