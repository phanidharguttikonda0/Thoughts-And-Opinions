...
public class UsersService {
    ...
    public ThoughtsResponse createThought(Thoughts.CreateRequest thoughtRequest) {
        ...
        if (content == null && parentThought != null) {
            boolean alreadyReposted = thoughtsRepo.existsByUserIdAndParentThoughtIdAndContentIsNull(user.get().getId(),
                    parentThought.getId());
            if (alreadyReposted) {
                log.warn("User {} already reposted thought {}", user.get().getId(), parentThought.getId());
                throw new com.thoughtsandopinions.thoughtsservice.exception.DuplicateRepostException(
                        "Already reposted this thought");
            }
        }
        ...
        // but we need id, so we are going with saveAndFlush to populate generated
        // fields like createdAt
        ThoughtsEntity createdThought = thoughtsRepo.saveAndFlush(thought);

        log.info("Successfully created thought with id: {} for userId: {}", createdThought.getId(),
                thoughtRequest.getUserId());
    ...
    public void deleteThought(Thoughts.DeleteRequest thoughtRequest) {
        log.info("Attempting to delete thoughtId: {} for userId: {}", thoughtRequest.getThoughtId(),
                thoughtRequest.getUserId());
        ...
        ThoughtsEntity thought = thoughtsRepo.findById(thoughtRequest.getThoughtId())
                .orElseThrow(() -> {
                    log.error("Failed to delete thought: Thought not found for thoughtId: {}",
                            thoughtRequest.getThoughtId());
        ...
        // from thoughts table and also likes , mentions and media attached to this
        // thought

        log.info("Successfully deleted thoughtId: {} for userId: {}", thoughtRequest.getThoughtId(),
                thoughtRequest.getUserId());
